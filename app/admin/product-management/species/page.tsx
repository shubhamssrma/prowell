...
import { createCategory, createSpecies, deleteCategory, deleteSpecies, getCategories, getProducts, getSpecies } from '@/store/slices/productSlice';
...
const CategoryPage = () => {
  ...
  const { species, loading, error, pagination, categoryLoading } = useAppSelector(state => state.productReducer)
  ...
  useEffect(() => {
    dispatch(getSpecies({ limit: itemsPerPage, page: currentPage, search: debouncedSearch }))
  ...
  const handleSubmit = async () => {
    ...
    if (editingCategory) {
      ...
      const data = await dispatch(createSpecies(formData)).unwrap()
      if (data.success) {
        await dispatch(getSpecies({ limit: itemsPerPage, page: currentPage, search: debouncedSearch }))
      }
      console.log(data)
    }
  ...
  const handleDeleteCategory = async (id: string) => {
    const resp = await dispatch(deleteSpecies({ id })).unwrap()
    if (resp.success) {
      await dispatch(getSpecies({ limit: itemsPerPage, page: currentPage, search: debouncedSearch }))
    }
  }
      ...
      <div className="min-h-screen bg-gray-50 p-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex justify-between items-start mb-6">
            <div>
              <h1 className="text-2xl font-bold mb-1">All Species</h1>
              <p className="text-gray-600 text-sm">Manage your species</p>
            </div>
            <div className="flex gap-3">
              ...
              >
                ...
                <span className="text-sm font-medium">Add Species</span>
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
            <div className="bg-white rounded-lg p-4 shadow-md">
              <div className="flex justify-between items-start mb-2">
                <span className="text-sm text-gray-600">Total Species</span>
          ...
          <p className="text-sm text-gray-600 mb-4">
            Showing {startIndex + 1}-{Math.min(endIndex, pagination.total)} of {pagination.total} Species
          </p>
            ...
            <div className="overflow-x-visible">
              <table className="w-full">
                <thead className="bg-gray-50 border border-gray-300 shadow-sm">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Species Name</th>
                  ...
                  {loading ?
                    <tr>
                      <td colSpan={8} className="py-16 text-center">
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <p className="text-gray-500 font-medium">Loading species...</p>
                        </div>
                      </td>
                    </tr>
                    :
                    species.length == 0 ?
                      <tr>
                        <td colSpan={8} className="py-16 text-center">
                          <div className="flex flex-col items-center justify-center">
                            <p className="text-black font-medium">No Category Found!</p>
                          </div>
                        </td>
                      </tr>
                      :
                ...
                <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
                  <div className="flex justify-between items-center p-6 border-b border-gray-200">
                    <h2 className="text-xl font-semibold text-gray-800">
                      {editingCategory ? 'Edit Species' : 'Add Species'}
                    </h2>
                    <button
                      ...
                      >
                        {
                          categoryLoading ?
                            ...
                            :
                            <>
                              {editingCategory ? 'Update' : 'Add'} Species
                            </>
                        }
                      </button>
                    </div>
            ...
            {showDeleteAlert && (
              <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4 p-6">
                  <h3 className="text-lg font-semibold text-gray-800 mb-3">Delete Species</h3>
