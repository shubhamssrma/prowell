...
import { createCategory, createRegion, deleteCategory, deleteRegion, getCategories, getProducts, getRegions } from '@/store/slices/productSlice';
...
const CategoryPage = () => {
  ...
  const { regions, loading, error, pagination, categoryLoading } = useAppSelector(state => state.productReducer)
  ...
  useEffect(() => {
    dispatch(getRegions({ limit: itemsPerPage, page: currentPage, search: debouncedSearch }))
  ...
  const handleSubmit = async () => {
    ...
    if (editingCategory) {
      ...
      const data = await dispatch(createRegion(formData)).unwrap()
      if (data.success) {
        await dispatch(getRegions({ limit: itemsPerPage, page: currentPage, search: debouncedSearch }))
      }
      console.log(data)
    }
  ...
  const handleDeleteCategory = async (id: string) => {
    const resp = await dispatch(deleteRegion({ id })).unwrap()
    if (resp.success) {
      await dispatch(getRegions({ limit: itemsPerPage, page: currentPage, search: debouncedSearch }))
    }
  }
      ...
      <div className="min-h-screen bg-gray-50 p-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex justify-between items-start mb-6">
            <div>
              <h1 className="text-2xl font-bold mb-1">All Regions</h1>
              <p className="text-gray-600 text-sm">Manage your regions</p>
            </div>
          ...
          <p className="text-sm text-gray-600 mb-4">
            Showing {startIndex + 1}-{Math.min(endIndex, pagination.total)} of {pagination.total} Regions
          </p>
            ...
            <div className="overflow-x-visible">
              <table className="w-full">
                <thead className="bg-gray-50 border border-gray-300 shadow-sm">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Region Name</th>
                  ...
                  {loading ?
                    ...
                    regions.length == 0 ?
                      <tr>
                        <td colSpan={8} className="py-16 text-center">
                          <div className="flex flex-col items-center justify-center">
                            <p className="text-black font-medium">No Region Found!</p>
                          </div>
                        </td>
                      </tr>
                      :
                ...
                <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
                  <div className="flex justify-between items-center p-6 border-b border-gray-200">
                    <h2 className="text-xl font-semibold text-gray-800">
                      {editingCategory ? 'Edit Region' : 'Add Region'}
                    </h2>
                    <button
                      ...
                      >
                        {
                          categoryLoading ?
                            ...
                            :
                            <>
                              {editingCategory ? 'Update' : 'Add'} Region
                            </>
                        }
                      </button>
                    </div>
            ...
            {showDeleteAlert && (
              <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4 p-6">
                  <h3 className="text-lg font-semibold text-gray-800 mb-3">Delete Region</h3>
